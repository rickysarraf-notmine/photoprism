...
func IndexMain(related *RelatedFiles, ind *Index, opt IndexOptions) (result IndexResult) {
	...
	if f.IsMotionPhoto() {
		log.Debugf("index: found motion photo %s", txt.Quote(f.FileName()))

		if mpFile, err := f.ExtractVideoFromMotionPhoto(); err != nil {
			log.Errorf("index: %s in %s (extract motion photo video)", err.Error(), txt.Quote(f.BaseName()))
		} else {
			log.Debugf("index: created motion photo video for %s", txt.Quote(mpFile.BaseName()))
